...
# def _windowed_subdivs_old(padded_img, window_size, subdivisions, nb_classes, pred_func):
#     """
...
def create_batches_4d_array(initial_array, batch_size):
    # Get the shape of the initial array
    total_tiles, height, width, channels = initial_array.shape
    
    # Initialize an empty list to store batches
    batches = []

    # Calculate the number of batches needed
    num_batches = total_tiles // batch_size

    # Iterate through the array and create batches
    for i in range(num_batches):
        start_idx = i * batch_size
        end_idx = (i + 1) * batch_size
        batch = initial_array[start_idx:end_idx]
        batches.append(batch)

    # If there are remaining tiles that don't form a complete batch, include them in a last batch
    if total_tiles % batch_size != 0:
        remaining_tiles = initial_array[num_batches * batch_size:]
        batches.append(remaining_tiles)
...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, batch_size, model):
    """
    Create tiled overlapping patches.
    ...
    gc.collect()
    a, b, c, d, e = subdivs.shape
    subdivs = subdivs.reshape(a * b, c, d, e) # contains all the tiles -> should be divided into batches before running model.predict
    
    batches = create_batches_4d_array(subdivs, batch_size)
    ...
    for image_batch in batches:
        ...
        pred_batches.append(pred_mask_batch)
    # print('sum pred_batch:')
    # print(np.sum(pred_batches[0][0][0][0]))
    #subdivs = pred_func(subdivs)
    subdivs = []
    subdivs = flatten_batches(pred_batches)
...
def predict_img_with_smooth_windowing(input_img, model, window_size, subdivisions, nb_classes, batch_size): #pred_function
    """
    ...
    for pad in tqdm(pads):
        # For every rotation:
        sd = _windowed_subdivs(pad, window_size, subdivisions, nb_classes, batch_size, model)
